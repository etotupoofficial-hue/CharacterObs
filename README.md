...
**WinError 10013 (доступ к сокету запрещён)** обычно означает, что соединение с
OBS WebSocket блокируется системой или порт занят. Проверьте следующее:

- OBS запущен и WebSocket включён.
- Порт в `CONFIG["obs_port"]` совпадает с настройкой OBS.
- Брандмауэр Windows разрешает подключение к `obs64.exe` и Python.
- Другие программы не занимают этот порт.
